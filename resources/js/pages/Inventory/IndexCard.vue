<template>
  <div class="p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-6">
      <div v-for="item in inventoryItems" :key="item.id" class="bg-white shadow rounded flex flex-col items-center">
        <img v-if="item.image" :src="'/storage/' + item.image" class="w-full h-32 object-cover rounded mb-2"
          alt="Inventory Image" />
        ...
        <div class="p-4 flex flex-col items-center">
          ...
          <div v-if="item.showDetails" class="w-full text-center">
            <p class="text-gray-500 mb-1">{{ item.description }}</p>
            <p class="text-gray-500 mb-1">Type: {{ item.inventory_type ? item.inventory_type.name : '-' }}</p>
            <p class="text-gray-500 mb-1">Supplier: {{ item.supplier ? item.supplier.name : '-' }}</p>
            <p class="text-gray-500 mb-1">Distributor: {{ item.distributor ? item.distributor.name : '-' }}</p>
            <p class="text-gray-500 mb-1">Unit: {{ item.unit }}</p>
            <p class="text-gray-500 mb-1">Qty: {{ item.current_quantity }}</p>
            <p class="text-gray-500 mb-1">Cost: {{ item.current_cost_price }}</p>
            <p class="text-gray-500 mb-2">Sale: {{ item.current_sale_price }}</p>

            <button @click="item.showDetails = false" class="text-purple-600 hover:underline mt-2">
              Show Less
            </button>
          </div>

          <div v-else>
            <button @click="item.showDetails = true" class="text-purple-600 hover:underline">
              Read More
            </button>
          </div>
